class PostsController < ApplicationController
  ...
  def create
    ...
    if @post.publish
      redirect_to @post, notice: t('headings.published_success')
    else
      @post.unpublish
      flash.now[:alert] = t('headings.create_error')
      render :new
    end
  end
  ...
  def update
    ...
    if @post.publish
      ...
      redirect_to @post, notice: t('headings.update_success')
    else
      @post.unpublish
      flash.now[:alert] = t('headings.update_error')
      render :edit
    end
  end
  ...
  def destroy
    @post.archived!
    redirect_to root_url, notice:  t('headings.archive_success')
  end
